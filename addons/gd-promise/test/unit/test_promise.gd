...
#
# Notes:
# - No single test triggers a hard engine error. Rejections are always handled
...
func test_finally_downstream_resolves_with_status() -> void:
	var noop := func(_s): pass
	var r: Array = await Promise.resolve("payload").finally_cb(noop).await_status()
	# Documented behavior: the next promise receives the STATUS, not the value.
	assert_eq(r[0], Promise.Status.RESOLVED)
	assert_eq(r[1], Promise.Status.RESOLVED)
...
func test_finally_return_value_is_discarded_resolves_with_status() -> void:
	# QUIRK (documented current behavior): finally_return's value never
	# reaches the chain, because finally_cb resolves with the status for any
	# non-Promise handler return. If this test starts failing, the library
	# semantics changed.
	var r: Array = await Promise.resolve("x").finally_return("ignored").await_status()
	assert_eq(r[0], Promise.Status.RESOLVED)
	assert_eq(r[1], Promise.Status.RESOLVED)
	# GUT can't compare INT to STRING, so assert the type instead of
	# assert_ne(r[1], "ignored") — same proof, valid comparison.
	assert_eq(typeof(r[1]), TYPE_INT, "value is the Status enum, not the string")
...
func test_cancelling_child_removes_queued_handlers_from_parent() -> void:
	var d := _deferred()
	var hit := [false]
	var child: Promise = d.promise.and_then(func(_v): hit[0] = true)
	child.cancel()
	d.resolve.call(1)
	assert_false(hit[0], "cancelled child's handler must not run")
...
func test_cancelling_parent_leaves_existing_children_pending() -> void:
	# DOCUMENTED CURRENT BEHAVIOR: _parent/_consumers are never wired up
	# anywhere in the library, so cancellation does NOT propagate to
	# already-attached children. The parent's queues are cleared on cancel,
	# leaving children permanently PENDING. If propagation is ever
	# implemented, update this test to expect CANCELLED.
	var d := _deferred()
	var child: Promise = d.promise.and_then(func(v): return v)
	d.promise.cancel()
	assert_eq(d.promise.get_status(), Promise.Status.CANCELLED)
	assert_eq(child.get_status(), Promise.Status.PENDING)
...
func test_all_rejects_on_first_rejection() -> void:
	...
	assert_eq(r[0], Promise.Status.REJECTED)
	assert_eq(r[1], "boom")
	d1.promise.cancel()  # tidy up the still-pending promise
